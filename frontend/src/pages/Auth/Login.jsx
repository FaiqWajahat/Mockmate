...
const Login = ({ setCurrentPage, onClose, isDark = false }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const message = error?.message || error?.code || "Google login failed. Please try again.";
      setError(message);
    } finally {
      setLoginLoading(false);
    }
  ...
  return (
    <div
      className="w-[90vw] md:w-[33vw] p-7 flex flex-col justify-start rounded-lg shadow  max-h-[95vh] h-auto  overflow-y-scroll"
      style={{
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div className="mt-3 flex items-center justify-center gap-3 w-full ">
          ...
          <label className="flex items-center cursor-pointer select-none">
            <input
              type="checkbox"
              checked={email === "guest@gmail.com" && password === "guest@1234"}
              onChange={e => {
                if (e.target.checked) {
            ...
            />
            <span
              className={`w-10 h-6 flex items-center bg-blue-200 rounded-full p-1 duration-300 ease-in-out ${email === "guest@gmail.com" && password === "guest@1234"
                ? "bg-blue-500"
                : "bg-blue-200"
                }`}
            >
              <span
                className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ease-in-out ${email === "guest@gmail.com" && password === "guest@1234"
                  ? "translate-x-4"
                  : ""
                  }`}
              />
